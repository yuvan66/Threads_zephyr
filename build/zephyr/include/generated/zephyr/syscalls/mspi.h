...
extern int z_impl_mspi_memmap_config(const struct device * controller, const struct mspi_dev_id * dev_id, const struct mspi_memmap_cfg * cfg);

static inline int mspi_memmap_config(const struct device * controller, const struct mspi_dev_id * dev_id, const struct mspi_memmap_cfg * cfg)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = controller };
		union { uintptr_t x; const struct mspi_dev_id * val; } parm1 = { .val = dev_id };
		union { uintptr_t x; const struct mspi_memmap_cfg * val; } parm2 = { .val = cfg };
		return (int) arch_syscall_invoke3(parm0.x, parm1.x, parm2.x, K_SYSCALL_MSPI_MEMMAP_CONFIG);
	}
#endif
	compiler_barrier();
	return z_impl_mspi_memmap_config(controller, dev_id, cfg);
}

#if defined(CONFIG_TRACING_SYSCALL)
#ifndef DISABLE_SYSCALL_TRACING

#define mspi_memmap_config(controller, dev_id, cfg) ({ 	int syscall__retval; 	sys_port_trace_syscall_enter(K_SYSCALL_MSPI_MEMMAP_CONFIG, mspi_memmap_config, controller, dev_id, cfg); 	syscall__retval = mspi_memmap_config(controller, dev_id, cfg); 	sys_port_trace_syscall_exit(K_SYSCALL_MSPI_MEMMAP_CONFIG, mspi_memmap_config, controller, dev_id, cfg, syscall__retval); 	syscall__retval; })
#endif
#endif
